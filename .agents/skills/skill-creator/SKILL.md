---
name: skill-creator
description: Guide for creating effective skills. This skill should be used when users want to create a new skill (or update an existing skill) that extends this repo's agent workflow with specialized knowledge, workflows, or tool integrations.
metadata:
...
Skills are modular, self-contained folders that extend this repo's agent workflow by providing
...
Claude Code loads FORMS.md, REFERENCE.md, or EXAMPLES.md only when needed.
...
When a user asks about sales metrics, Claude Code only reads sales.md.
...
When the user chooses AWS, Claude Code only reads aws.md.
...
Claude Code reads REDLINING.md or OOXML.md only when the user needs those features.
...
- "Where should I create this skill? If you do not have a preference, I will place it in `$CODEX_HOME/skills` (or `~/.codex/skills` when `CODEX_HOME` is unset) so Claude Code can discover it automatically."
