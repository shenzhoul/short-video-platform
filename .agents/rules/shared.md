...
- **Never import `react-toastify` directly in application code.** The only container is `SharedToastProvider` and it registers with a `containerId`; react-toastify delivers a toast only to a container whose id matches, so a bare call is dispatched to a container that does not exist and renders **nothing at all** — no error, no warning, just silence where the user should have been told something. This went unnoticed across 34 files until 2026-08-23. `user/eslint.config.js` now enforces it with `no-restricted-imports` (module and subpaths, including the stylesheet, which the provider owns), and `user/src/lib/toast-import-rule.spec.ts` runs ESLint for real to prove the rule fails. Mirror the rule into `admin/` if a direct import ever appears there.
- A spec that mocks a module the app no longer imports is inert and silently stops testing anything. When a shared package replaces a direct dependency, grep the specs for `jest.mock('<old-module>')` in the same change.
- `shared/*` packages are wired with `"file:../shared/<name>"`, and **Yarn v1 copies rather than links them**. Editing a shared package does not reach the apps: `yarn install` and even `--check-files` report "already up-to-date" and skip the re-copy. Delete `node_modules/@douyin-clone/<name>` and run `yarn install --force` in every consuming app, then verify the copy actually changed before concluding a shared-package edit had no effect.
