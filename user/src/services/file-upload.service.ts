 ...
 * const tusResult = await fileUploadService.uploadFileWithTUS(file, uploadData, {
...
export class FileUploadService extends APIRequest {
  ...
  */
  ...
  async uploadFileTus(
    ...
    // Load TUS only when a browser upload starts. Its Node lock dependency
    // installs process signal listeners and must stay out of Next dev HMR.
    ...
    return new Promise((resolve) => {
      try {
        ...
        const upload = new Upload(file, {
          ...
          onChunkComplete: (_chunkSize: number, _bytesAccepted: number, _bytesTotal: number) => {
            // Chunk upload completed - could be used for progress tracking
          }
        });
...
export const uploadFile = (
  ...
  onProgress?: (progress: UploadProgress) => void,
  onPrepared?: (prepared: UploadPrepared) => void
) => fileUploadServiceInstance.uploadFile(endpoint, file, options, onProgress, onPrepared);
