...
exports.encryptPassword = (pw, salt) => {
  ...
  // Password is already hashed on client-side, just add salt for storage
...
exports.createSetting = async (data) => {
  const exist = await this.DB.collection('settings').findOne({ key: data.key });
  if (exist) return exist;
  return this.DB.collection('settings').insertOne({
    ...data,
    type: data.type || 'text',
    createdAt: new Date(),
    updatedAt: new Date()
...
exports.createSettings = async (items) => {
  await items.reduce(async (cb, data) => {
    await cb;
    const exist = await this.DB.collection('settings').findOne({ key: data.key });
    if (exist) return exist;
...
exports.upsertSetting = async (data) => {
  ...
  const exist = await settings.findOne({ key: data.key });
